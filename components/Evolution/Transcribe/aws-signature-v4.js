...
export const createCanonicalRequest = (method, pathname, query, headers, payload) => {
  return [
    method.toUpperCase(),
    pathname,
...
export const createCanonicalQueryString = (params) => {
  return Object.keys(params).sort().map(function(key) {
    return encodeURIComponent(key) + '=' + encodeURIComponent(params[key]);
  }).join('&');
};

export const createCanonicalHeaders = (headers) => {
  return Object.keys(headers).sort().map(function(name) {
    return name.toLowerCase().trim() + ':' + headers[name].toString().trim() + '\n';
  }).join('');
};

export const createSignedHeaders = (headers) => {
  return Object.keys(headers).sort().map(function(name) {
    return name.toLowerCase().trim();
  }).join(';');
};

export const createCredentialScope = (time, region, service) => {
  return [toDate(time), region, service, 'aws4_request'].join('/');
};

export const createStringToSign = (time, region, service, request) => {
  return [
    'AWS4-HMAC-SHA256',
    toTime(time),
...
export const createSignature = (secret, time, region, service, stringToSign) => {
...
export const createPresignedS3URL = (name, options) => {
  options = options || {};
...
export const createPresignedURL = (method, host, path, service, payload, options) => {
  options = options || {};
