...
const Speak = () => {
  ...
  const { firstAlarmAnnounced, transferOfCommandCompleted } = useSelector(
    (state) => state.ai
  )
  const { waitingToBeSpoken, radioInUse } = useSelector((state) => state.units)
  ...
  useEffect(() => {
    ...
    if (!commandInProgress && !radioInUse && waitingToBeSpoken.length > 0) {
      const waitTime =
        firstAlarmAnnounced && !transferOfCommandCompleted ? 2000 : 10
      cooldown = setTimeout(() => {
