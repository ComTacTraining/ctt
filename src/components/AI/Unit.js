...
import { addToSpeechQueue, incrementUnitsAssigned, addAssignedGroup, incidentAnnounced } from 'store/actions/ai';
import { options, anyTermsMatchString, randomSelection, properPronouns } from 'utils/ai';
...
const Unit = ({name, voice, index}) => {
  ...
  const { command, incidentCommandName, unitsAssigned } = useSelector(state => state.ai);
  const { 
    incidentGroup, 
    incidentCommand, 
    withstanding, 
  ...
  useEffect(() => {
    ...
    if (unitName) {
      ...
      if (index === 0) {
        ...
        timeout = Math.floor(
          Math.random() * (maxUnitArrivalSeconds - minUnitArrivalSeconds + 1) + minUnitArrivalSeconds
        );
        timeout *= 1000;
      } 
      interval = setTimeout(() => {
  ...
  useEffect(() => {
    const checkForNeeds = () => {
      switch (icsNimsGroup) {
        case 'Fire Attack': return attack;
    ...
    const checkForCanReport = () => {
      if (anyTermsMatchString(command, canReportTerms)) {
        ...
        const canReport = group.canReports.find((report) => report.type === type).response;
  ...
  }, [icsNimsGroup, command, unitName, withstanding, attack, ventilation, exposure, ric, medical]);

  useEffect(() => {
    const normalizedGroup = () => {
      let group = '';
      switch (incidentGroup) {
        case 'FIRE_ATTACK': group = 'Fire Attack'; break;
        case 'VENTILATION': group = 'Ventilation'; break;
        case 'EXPOSURE': group = 'Exposure'; break;
        case 'RIC': group = 'RIC'; break;
        case 'MEDICAL': group = 'Medical'; break;
        default: break;
      }
      return group;
    }
