...
const TextField = () => {
  ...
  const handleCompletedCommand = (evt) => {
    ...
    dispatch(addToLog({ timestamp: Date.now(), label: firstOnScene, text: currentCommand }))
    setCurrentCommand('');
  }

  return (
    <form onSubmit={handleCompletedCommand}>
      <MUITextField fullWidth={true} multiline rowsMax={4} id="command" label="Command" value={currentCommand} onChange={handlePartialCommand} />
