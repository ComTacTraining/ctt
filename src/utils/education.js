...
const formatConstruction = (construction) => {
  return replaceUnderscores(construction.join(', ').replace('TILT_UP', 'TILT-UP'));
}

const formatSize = (size) => {
  return replaceUnderscores(size);
}

const formatStories = (stories) => {
  return (stories === 1) ? 'single story' : `${stories} story`;
}

const formatSingleLocation = (location) => {
  return replaceUnderscores(location);
}

const formatMultipleLocations = (locations) => {
  return replaceUnderscores(locations.join(', '));
}

const formatFire = (fire) => {
  return fire.replace('_', ' & ');
}

const getPrimarySizeUp = ({ firstOnScene, size, stories, occupancy, conditions }) => {
  return `${firstOnScene} on scene of a ${formatSize(size)} ${formatStories(stories)} ${occupancy} with smoke and fire showing from the ${formatSingleLocation(conditions)} side.`
}

const getSecondarySizeUp = ({ construction, entryEgress, survivability }) => {
  return `This is a typical ${formatConstruction(construction)} construction, with entry and egress points on ${formatMultipleLocations(entryEgress)} sides. The survivability profile is ${survivability} throughout the structure.`;
};

const getLocationPhrase = ({ fire, exhaust }) => {
  return `The fire appears to be a ${formatFire(fire)} fire on the ${formatSingleLocation(exhaust)} side based on smoke & fire conditions.`;
};

const getExhaustPhrase = ({ flow, exhaust }) => {
  if (flow === 'UNI_DIRECTIONAL') {
    return `The flow path is Uni-directional with the exhaust on the ${formatSingleLocation(exhaust)} side. Uni-Directional is when the intake is from one opening and the exhaust is from a separate opening. If you control the airflow, you control the fire intensity. By using smoke indicators and fire conditions, control the flow path by managing vertical openings, doors and windows. These actions will limit fire growth, thus allowing longer offensive fire operations.`;
  } else {
    return `The flow path is Bi-Directional with the exhaust path on the ${formatSingleLocation(exhaust)} side. Bi-Directional is where the intake and exhaust is from the same opening. Control the airflow controls the fires intensity. If you control the airflow, you control the fire intensity. By using smoke indicators and fire conditions, control the flow path by managing vertical openings, doors and windows. These actions will limit fire growth, thus allowing longer offensive fire operations.`;
  }
};

const getSmokePhrase = ({ smoke }) => {
  switch (smoke) {
    case 'GRAY_LAMINAR': 
...
const getCategoryPhrases = ({ category }) => {
  switch (category) {
    case 'COMMERCIAL_LEGACY': 
    case 'INDUSTRIAL_LEGACY':
      return [
...
const getEducationPhrases = ({ 
  ...
  const primarySizeUp = getPrimarySizeUp({ firstOnScene, size, stories, occupancy, conditions });
  const secondarySizeUp = getSecondarySizeUp({ construction, entryEgress, survivability });
  const locationPhrase = getLocationPhrase({ fire, exhaust });
  const exhaustPhrase = getExhaustPhrase({ flow, exhaust });
  const smokePhrase = getSmokePhrase({ smoke });
  const categoryPhrases = getCategoryPhrases({ category });
  let phrases = [
  ...
  phrases.push('You have now completed the education phase of this training evolution. We will now proceed to the evaluation phase.');
  return phrases;
};
