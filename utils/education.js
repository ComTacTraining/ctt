...
const formatConstruction = construction => {
  return replaceUnderscores(
    construction.join(", ").replace("TILT_UP", "TILT-UP")
  );
};
...
const getPrimarySizeUp = ({
  ...
  return `${firstOnScene} on scene of a ${formatSize(size)} ${formatStories(
    stories
  )} ${occupancy} with smoke and fire showing from the ${formatSingleLocation(
    conditions
  )} side.`;
};

const getSecondarySizeUp = ({ construction, entryEgress, survivability }) => {
  return `This is a typical ${formatConstruction(
    construction
  )} construction, with entry and egress points on ${formatMultipleLocations(
    entryEgress
  )} sides. The survivability profile is ${survivability} throughout the structure.`;
};

const getLocationPhrase = ({ fire, exhaust }) => {
  return `The fire appears to be a ${formatFire(
    fire
  )} fire on the ${formatSingleLocation(
    exhaust
...
const getExhaustPhrase = ({ flow, exhaust }) => {
  if (flow === "UNI_DIRECTIONAL") {
    return `The flow path is Uni-directional with the exhaust on the ${formatSingleLocation(
      exhaust
    ...
    return `The flow path is Bi-Directional with the exhaust path on the ${formatSingleLocation(
      exhaust
...
const getEducationPhrases = ({
  ...
  const primarySizeUp = getPrimarySizeUp({
    firstOnScene,
    size,
    stories,
    occupancy,
  ...
  const secondarySizeUp = getSecondarySizeUp({
    construction,
    entryEgress,
    survivability
  });
