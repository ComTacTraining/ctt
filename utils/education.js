const formatConstruction = (construction) => {
  return construction
    .join(', ')
    .toLowerCase()
    .replace('concretetiltup', 'concrete tilt-up')
    .replace('metalclad', 'metal clad')
}
...
const formatFire = (fire) => {
  if (fire === 'ROOMCONTENTS') {
    return 'room and contents'
  }
  return fire.toLowerCase()
}
...
const getSecondarySizeUp = ({ construction, entryEgress, survivability }) => {
  return `This is a typical ${formatConstruction(
    ...
    entryEgress
  )} sides. The survivability profile is ${survivability.toLowerCase()} throughout the structure.`
}
